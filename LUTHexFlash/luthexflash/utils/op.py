...
def unpack_gptqv2(
    qweight: np.ndarray, scales: np.ndarray, qzeros: np.ndarray, gptq_v2: bool = True
):
    """
    Unpack GPTQv2
    ...
    # Unpack qweight
    qweights = [
        (qweight >> bit_offset) & ((1 << bits) - 1) for bit_offset in range(0, 32, bits)
    ]
    ...
    # Unpack qzeros
    zeros = [
        (qzeros >> bit_offset) & ((1 << bits) - 1) for bit_offset in range(0, 32, bits)
    ]
...
def hvx_preprocess_weights(
    ...
    assert (
        ...
        or scales.dtype == "float32"
        or scales.dtype == "bfloat16"
    )
    ...
    #             0            1            2                3      4                5
    w = w.reshape(
        P // tile_p, Q // tile_q, tile_p // vec_p, vec_p, tile_q // vec_q, vec_q
    ...
    w = (
        w.reshape(-1, vec_q, vec_p)
        .reshape(-1, vec_q // 2, 2, vec_p)
        .transpose(0, 1, 3, 2)
    )
    w = sum([(w[:, :, :, n] << (n * g)) for n in range(2)])
    w = w.reshape(
        P // tile_p, Q // tile_q, tile_p // vec_p, tile_q // vec_q, vec_q // 2, vec_p
    )
    ...
    if scales.size >= M:  # GPTQ
        ...
        scales = scales.reshape(
            P // tile_p, tile_p // bits, Q // tile_q, tile_q // q_group_size
        ).transpose(0, 2, 1, 3)
        #                       0            1            2                        3      4
        scales = scales.reshape(
            P // tile_p,
