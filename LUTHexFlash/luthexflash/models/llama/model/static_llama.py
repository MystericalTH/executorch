...
from luthexflash.models.llama.masking_utils import (
    AttentionMask,
    CausalAttentionMask,
    SlidingWindowAttentionMask,
)
from luthexflash.models.llama.model import (
    NORM_REGISTRY,
    ROTARY_EMB_REGISTRY,
...
def permute(qlinear: nn.Module, n_head: int, n_head_kv: int | None):
    ...
    def _permute(weights: torch.Tensor, n_head: int, n_head_kv: int | None):
        ...
        return (
            weights.reshape(
                weights.shape[0], n_head, 2, weights.shape[1] // n_head // 2
            )
            .swapaxes(-2, -1)
    ...
    qzeros = np.zeros(
        (zeros.shape[0], zeros.shape[1] // qlinear.pack_dtype_bits * qlinear.bits),
        dtype=qlinear.pack_np_math_dtype,
    )
    for col in range(qzeros.shape[1]):
        for j in range(qlinear.pack_factor):
            qzeros[:, col] |= zeros[:, col * qlinear.pack_factor + j] << (
                qlinear.bits * j
            )
...
class LlamaAttention(nn.Module):
    ...
    def prepare_tman(self, do_permute=False, use_sha=False):
        ...
        assert isinstance(self.wq, TorchLinear)
        assert isinstance(self.wk, TorchLinear)
        assert isinstance(self.wv, TorchLinear)
        assert isinstance(self.wo, TorchLinear)
        self.wq.post_init()
        self.wk.post_init()
        self.wv.post_init()
        self.wo.post_init()
        ...
        if do_permute:
            wq_qweight, wq_scales, wq_qzeros = permute(
                self.wq, self.n_heads, self.n_heads
            )
            wk_qweight, wk_scales, wk_qzeros = permute(
                self.wk, self.n_heads, self.n_kv_heads
            )
        else:
            wq_qweight, wq_scales, wq_qzeros = (
                self.wq.qweight,
                self.wq.scales,
                self.wq.qzeros,
            )
            wk_qweight, wk_scales, wk_qzeros = (
                self.wk.qweight,
                self.wk.scales,
                self.wk.qzeros,
            )
        wv_qweight, wv_scales, wv_qzeros = (
            self.wv.qweight,
            self.wv.scales,
            self.wv.qzeros,
        )
        ...
        def _copy(
            target: nn.ModuleList | TMANLinear,
            source: TorchLinear,
            ...
            if use_sha:
                ...
                for i in range(n_head):
                    target[i].qweight.copy_(
                        qweight[:, i * self.head_dim : (i + 1) * self.head_dim]
                    )
                    target[i].scales.copy_(
                        scales[:, i * self.head_dim : (i + 1) * self.head_dim]
                    )
                    target[i].qzeros.copy_(
                        qzeros[
                            :,
                            i * self.head_dim // source.pack_factor : (i + 1)
                            * self.head_dim
                            // source.pack_factor,
                        ]
                    )
    ...
    def forward_tman(
        ...
        if isinstance(self.wq_tman, nn.ModuleList):
            q = [wq_tman(hidden_states) for wq_tman in self.wq_tman]
            k = [wk_tman(hidden_states) for wk_tman in self.wk_tman]
            v = [wv_tman(hidden_states) for wv_tman in self.wv_tman]
            for i in range(len(q)):
                q[i] = self.apply_rope_emb(q[i], freqs_cos, freqs_sin)
            for i in range(len(k)):
                k[i] = self.apply_rope_emb(k[i], freqs_cos, freqs_sin).permute(0, 2, 1)
        else:
            q = self.wq_tman(hidden_states).reshape(
                bsz, seq_len, self.n_heads, self.head_dim
            )
            k = self.wk_tman(hidden_states).reshape(
                bsz, seq_len, self.n_kv_heads, self.head_dim
            )
            v = self.wv_tman(hidden_states).reshape(
                bsz, seq_len, self.n_kv_heads, self.head_dim
            )
...
class MultiScopeAwareLlamaModel(LlamaModel):
    def __init__(
        ...
        **kwargs,
        ...
        assert config.local_rope_theta is not None, (
