...
DECLARE_PKG_OPS_OPTS_LIST(PKG_HexFlashAttentionQFinalize)
 ...
 * can be set by Qnn_ErrorHandle_t HexFlashAttentionV1LogSetLevel(QnnLog_Level_t
 * maxLogLevel)
 */
/*
...
Qnn_ErrorHandle_t HexFlashAttentionV1Init(
...
Qnn_ErrorHandle_t HexFlashAttentionV1GetInfo(const QnnOpPackage_Info_t** info) {
  if (!sg_packageInitialized)
...
Qnn_ErrorHandle_t HexFlashAttentionV1LogInitialize(
...
Qnn_ErrorHandle_t HexFlashAttentionV1LogSetLevel(QnnLog_Level_t maxLogLevel) {
  if (maxLogLevel < QNN_LOG_LEVEL_ERROR)
    return QNN_LOG_ERROR_INVALID_ARGUMENT;
  sg_maxLogLevel = maxLogLevel;
  return QNN_SUCCESS;
...
Qnn_ErrorHandle_t HexFlashAttentionV1LogTerminate() {
  if (!sg_logInitialized)
    return QNN_OP_PACKAGE_ERROR_LIBRARY_NOT_INITIALIZED;
...
Qnn_ErrorHandle_t HexFlashAttentionV1ValidateOpConfig(Qnn_OpConfig_t opConfig) {
  ...
  } else if (std::string(opConfig.v1.typeName) == "HexFlashAttentionQLocal") {
    if (opConfig.v1.numOfParams != 1 || opConfig.v1.numOfInputs != 4 ||
        opConfig.v1.numOfOutputs != 1) {
      return QNN_OP_PACKAGE_ERROR_VALIDATION_FAILURE;
    }
  } else if (
      std::string(opConfig.v1.typeName) == "HexFlashAttentionQFinalize") {
 ...
 * HexFlashAttentionV1CreateKernels (QnnOpPackage_GraphInfrastructure_t
 ...
 * HexFlashAttentionV1FreeKernels (QnnOpPackage_Kernel_t* kernels)
 *
 * (version 1.4.0) Qnn_ErrorHandle_t HexFlashAttentionV1CreateOpImpl
 ...
 * HexFlashAttentionV1FreeOpImpl (QnnOpPackage_OpImpl_t opImpl)
 */

Qnn_ErrorHandle_t HexFlashAttentionV1Terminate() {
  if (!sg_packageInitialized)
    return QNN_OP_PACKAGE_ERROR_LIBRARY_NOT_INITIALIZED;
...
/* latest version */
Qnn_ErrorHandle_t HexFlashAttentionV1InterfaceProvider(
    QnnOpPackage_Interface_t* interface) {
  if (!interface)
    return QNN_OP_PACKAGE_ERROR_INVALID_ARGUMENT;
  interface->interfaceVersion = {1, 4, 0};
  interface->v1_4.init = HexFlashAttentionV1Init;
  interface->v1_4.terminate = HexFlashAttentionV1Terminate;
  interface->v1_4.getInfo = HexFlashAttentionV1GetInfo;
  interface->v1_4.validateOpConfig = HexFlashAttentionV1ValidateOpConfig;
  interface->v1_4.createOpImpl = nullptr;
  interface->v1_4.freeOpImpl = nullptr;
  interface->v1_4.logInitialize = HexFlashAttentionV1LogInitialize;
  interface->v1_4.logSetLevel = HexFlashAttentionV1LogSetLevel;
  interface->v1_4.logTerminate = HexFlashAttentionV1LogTerminate;
  return QNN_SUCCESS;
}
