...
API_EXPORT QuickShape simpledim_chunk1_4d_split_start(
    Replacement& rpx,
...
API_EXPORT QuickShape simpledim_chunk1_4d_split_size(
    Replacement& rpx,
    Split_Context const& splitinfo,
    OpRef const& orig,
    int dim) {
...
#define SIZE_OF(WEIGHT)       \
  MUL(ELEMENTSIZE_OF(WEIGHT), \
      DIM_OF(WEIGHT, 0),      \
      DIM_OF(WEIGHT, 1),      \
      DIM_OF(WEIGHT, 2),      \
      DIM_OF(WEIGHT, 3))
...
DEF_PACKAGE_OPTIMIZATION(
    ...
    AUTOSPLIT(
        3,
        "I",
        DIV(DIM_OF("*", 3), DIM_OF("qweight", 2)),
        Op("TMANLinear",
           "l",
           AUTOSPLIT_SLICE(
               "qweight",
               AUTOSPLIT_SHAPEFN_APPLY(
                   simpledim_chunk1_4d_split_start,
                   "I",
                   "qweight",
                   2),
               AUTOSPLIT_SHAPEFN_APPLY(
                   simpledim_chunk1_4d_split_size,
                   "I",
                   "qweight",
                   2)),
           AUTOSPLIT_SLICE(
...
GraphStatus tmanlinearImpl(
  ...
  const int32_t gemm_k =
      qweight.dims()[2] * qweight.dims()[3] * 32 / bits / gemm_m;

  const int32_t l_size = gemm_k / LUT_G * LUT_SIZE;
  const int32_t ls_size =
      (ACT_GROUP_SIZE == -1) ? 1 : (gemm_k / ACT_GROUP_SIZE);
  ...
  if (zero_point && bits == 2 && group_size == 64) {
    ...
    hvx_tbl<
        ...
        LUT_G,
        true>(
        gemm_m, gemm_k, gemm_n, l_ptr, ls_ptr, lb_ptr, w_ptr, s_ptr, c_ptr);
  } else if (!zero_point && bits == 4 && group_size == 128) {
    // w4g128,
    // symmetric=True
    hvx_tbl<
        LType,
    ...
    hvx_tbl<
        ...
        true>(
        gemm_m, gemm_k, gemm_n, l_ptr, ls_ptr, lb_ptr, w_ptr, s_ptr, c_ptr);
  } else if (zero_point && bits == 4 && group_size == 64) {
    // w4g64,
    // symmetric=False
    hvx_tbl<
        LType,
        ...
        true>(
        gemm_m, gemm_k, gemm_n, l_ptr, ls_ptr, lb_ptr, w_ptr, s_ptr, c_ptr);
  } else if (!zero_point && bits == 2 && group_size == 0) {
    // bitnet
    hvx_tbl<LType, XType, CType, -1, 0, false, 2, TILE_K, LUT_G, true>(
        gemm_m, gemm_k, gemm_n, l_ptr, ls_ptr, lb_ptr, w_ptr, s_ptr, c_ptr);
  } else {
