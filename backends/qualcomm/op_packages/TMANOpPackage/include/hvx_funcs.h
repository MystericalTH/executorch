...
static HVX_INLINE_ALWAYS void l2fetch(
    const void* p,
    uint32_t stride,
    uint32_t width,
    uint32_t height,
...
inline typename std::enable_if_t<
    std::is_same<LType, int16_t>::value && std::is_same<XType, __fp16>::value,
    int>
hvx_lut_ctor(
    int32_t GemmK,
    int32_t GemmN,
  ...
  // Q groups with g activations each
  const int32_t Q = GemmK / g;

  // q_act_group_size LUT groups per activation scale
  const int32_t q_act_group_size =
      (ActGroupSize < 0) ? (Q / -ActGroupSize) : (ActGroupSize / g);
  // q_group_size LUT groups per group
  ...
  // lut_bias is stored in fp16 if ZeroPoint is true to avoid conversion during
  // hvx_tbl
  ...
  for (int32_t group_q = 0; group_q < Q; group_q += q_act_group_size) {
    ...
    for (int32_t q = 0; q < q_act_group_size; q += VecQ) {
      ...
      HVX_Vector sum_abs =
          Q6_Vhf_equals_Vqf16(Q6_Vqf16_vadd_Vqf16Vqf16(x01_abs, x23_abs));
    ...
    for (int32_t i = VLEN / 2; i >= 2; i >>= 1) {
      ls_val_vec = Q6_Vhf_vmax_VhfVhf(
          ls_val_vec, Q6_V_vlalign_VVR(ls_val_vec, zero_vec, i));
    }
    ...
    HVX_Vector tls_val_qf32 =
        Q6_Vqf32_vadd_VsfVsf(Q6_V_vsplat_R(_fp32_to_bits(tls_val)), zero_vec);
...
#pragma unroll
      for (int32_t i = 1; i < lut_size; i += 2) {
        if (i & 0b0010) {
          l_tmp_lo_qf32[i] =
              Q6_Vqf32_vadd_Vqf32Vqf32(Q6_V_lo_W(x0_qf32), Q6_V_lo_W(x1_qf32));
          l_tmp_hi_qf32[i] =
              Q6_Vqf32_vadd_Vqf32Vqf32(Q6_V_hi_W(x0_qf32), Q6_V_hi_W(x1_qf32));
        } else {
          l_tmp_lo_qf32[i] =
              Q6_Vqf32_vsub_Vqf32Vqf32(Q6_V_lo_W(x0_qf32), Q6_V_lo_W(x1_qf32));
          l_tmp_hi_qf32[i] =
              Q6_Vqf32_vsub_Vqf32Vqf32(Q6_V_hi_W(x0_qf32), Q6_V_hi_W(x1_qf32));
        }
        if (i & 0b0100) {
          l_tmp_lo_qf32[i] =
              Q6_Vqf32_vadd_Vqf32Vqf32(l_tmp_lo_qf32[i], Q6_V_lo_W(x2_qf32));
          l_tmp_hi_qf32[i] =
              Q6_Vqf32_vadd_Vqf32Vqf32(l_tmp_hi_qf32[i], Q6_V_hi_W(x2_qf32));
        } else {
          l_tmp_lo_qf32[i] =
              Q6_Vqf32_vsub_Vqf32Vqf32(l_tmp_lo_qf32[i], Q6_V_lo_W(x2_qf32));
          l_tmp_hi_qf32[i] =
              Q6_Vqf32_vsub_Vqf32Vqf32(l_tmp_hi_qf32[i], Q6_V_hi_W(x2_qf32));
        }
        if (i & 0b1000) {
          l_tmp_lo_qf32[i] =
              Q6_Vqf32_vadd_Vqf32Vqf32(l_tmp_lo_qf32[i], Q6_V_lo_W(x3_qf32));
          l_tmp_hi_qf32[i] =
              Q6_Vqf32_vadd_Vqf32Vqf32(l_tmp_hi_qf32[i], Q6_V_hi_W(x3_qf32));
        } else {
          l_tmp_lo_qf32[i] =
              Q6_Vqf32_vsub_Vqf32Vqf32(l_tmp_lo_qf32[i], Q6_V_lo_W(x3_qf32));
          l_tmp_hi_qf32[i] =
              Q6_Vqf32_vsub_Vqf32Vqf32(l_tmp_hi_qf32[i], Q6_V_hi_W(x3_qf32));
        }
      }
...
#pragma unroll
      for (int32_t i = 0; i < lut_size; i += 2) {
        ...
        l_tmp_lo_qf32[i] =
            Q6_Vqf32_vsub_Vqf32Vqf32(zero_vec, l_tmp_lo_qf32[lut_size - 1 - i]);
        l_tmp_hi_qf32[i] =
            Q6_Vqf32_vsub_Vqf32Vqf32(zero_vec, l_tmp_hi_qf32[lut_size - 1 - i]);
      }

      lb_val_vec =
          Q6_Vqf32_vadd_Vqf32Vqf32(lb_val_vec, l_tmp_lo_qf32[lut_size - 1]);
      lb_val_vec =
          Q6_Vqf32_vadd_Vqf32Vqf32(lb_val_vec, l_tmp_hi_qf32[lut_size - 1]);
...
#pragma unroll
      for (int32_t i = 0; i < lut_size; i += 1) {
        HVX_Vector l_tmp_lo =
            Q6_Vqf32_vmpy_Vqf32Vqf32(l_tmp_lo_qf32[i], tls_val_qf32);
        HVX_Vector l_tmp_hi =
            Q6_Vqf32_vmpy_Vqf32Vqf32(l_tmp_hi_qf32[i], tls_val_qf32);
...
#pragma unroll
      for (int32_t i = 0; i < lut_size / 2; i += 2) {
        l_pb[i + 0] =
            Q6_W_vshuff_VVR(Q6_V_lo_W(l_pa[i + 1]), Q6_V_lo_W(l_pa[i + 0]), -8);
        l_pb[i + 1] =
            Q6_W_vshuff_VVR(Q6_V_hi_W(l_pa[i + 1]), Q6_V_hi_W(l_pa[i + 0]), -8);
      }
      // 128-bit, interval=4
#pragma unroll
      for (int32_t i = 0; i < lut_size / 2; i += 4) {
        l_pa[i + 0] = Q6_W_vshuff_VVR(
            Q6_V_lo_W(l_pb[i + 2]), Q6_V_lo_W(l_pb[i + 0]), -16);
        l_pa[i + 1] = Q6_W_vshuff_VVR(
            Q6_V_hi_W(l_pb[i + 2]), Q6_V_hi_W(l_pb[i + 0]), -16);
        l_pa[i + 2] = Q6_W_vshuff_VVR(
            Q6_V_lo_W(l_pb[i + 3]), Q6_V_lo_W(l_pb[i + 1]), -16);
        l_pa[i + 3] = Q6_W_vshuff_VVR(
            Q6_V_hi_W(l_pb[i + 3]), Q6_V_hi_W(l_pb[i + 1]), -16);
      }
      // 256-bit, interval=8
#pragma unroll
      for (int32_t i = 0; i < lut_size / 2; i += 8) {
        l_pb[i + 0] = Q6_W_vshuff_VVR(
            Q6_V_lo_W(l_pa[i + 4]), Q6_V_lo_W(l_pa[i + 0]), -32);
        l_pb[i + 1] = Q6_W_vshuff_VVR(
            Q6_V_hi_W(l_pa[i + 4]), Q6_V_hi_W(l_pa[i + 0]), -32);
        l_pb[i + 2] = Q6_W_vshuff_VVR(
            Q6_V_lo_W(l_pa[i + 5]), Q6_V_lo_W(l_pa[i + 1]), -32);
        l_pb[i + 3] = Q6_W_vshuff_VVR(
            Q6_V_hi_W(l_pa[i + 5]), Q6_V_hi_W(l_pa[i + 1]), -32);
        l_pb[i + 4] = Q6_W_vshuff_VVR(
            Q6_V_lo_W(l_pa[i + 6]), Q6_V_lo_W(l_pa[i + 2]), -32);
        l_pb[i + 5] = Q6_W_vshuff_VVR(
            Q6_V_hi_W(l_pa[i + 6]), Q6_V_hi_W(l_pa[i + 2]), -32);
        l_pb[i + 6] = Q6_W_vshuff_VVR(
            Q6_V_lo_W(l_pa[i + 7]), Q6_V_lo_W(l_pa[i + 3]), -32);
        l_pb[i + 7] = Q6_W_vshuff_VVR(
            Q6_V_hi_W(l_pa[i + 7]), Q6_V_hi_W(l_pa[i + 3]), -32);
      }
      // write back
...
#pragma unroll
      ...
      if ((q_group_size >= VecQ) &&
          ((group_q + q) % q_group_size == (q_group_size - VecQ))) {
        // self_sum
        for (int32_t i = VLEN / 2; i >= 4; i >>= 1) {
          lb_val_vec = Q6_Vqf32_vadd_Vqf32Vqf32(
              lb_val_vec, Q6_V_vlalign_VVR(lb_val_vec, zero_vec, i));
        }
        vmem(tmp_buf) = Q6_Vsf_equals_Vqf32(lb_val_vec);
        lb_p[(group_q + q) / q_group_size] =
            -((const float*)tmp_buf)[VLEN / 4 - 1] * 0.5f;
        lb_val_vec = zero_vec;
      }
      if (q_group_size < VecQ) {
        ...
        for (int32_t i = VLEN / 4 - 1; i >= 0; i -= sum_len / 4) {
          lb_p[(group_q + q) / q_group_size + i / (sum_len / 4)] =
              -((const float*)tmp_buf)[i] * 0.5f;
        }
        lb_val_vec = zero_vec;
      }
    } // q_act_group_size
...
inline typename std::enable_if_t<
    std::is_same<LType, int16_t>::value && std::is_same<XType, __fp16>::value &&
        std::is_same<CType, float>::value && (GroupSize > 0),
    int>
hvx_tbl(
    int32_t GemmM,
    int32_t GemmK,
  ...
  // Q = K / g, P = M * Bits
  // x_shape: (Q / TileQ, TileQ / VecQ, VecQ, lut_size) = (Q, lut_size),
  // elem_size = 2 bytes w_shape: (P / TileP, Q / TileQ, TileP / VecP, TileQ /
  // VecQ, VecQ, VecP) indices, elem_size = g / 8 = 0.5 bytes indices of two
  // VecQ are zipped into one Vector
  const int32_t Q = GemmK / g;
  ...
  // In practice, for int16_t activation, group size < act group size (not
  // required)
  static_assert(
      (ActGroupSize % GroupSize == 0) || (GroupSize % ActGroupSize) == 0,
      "ActGroupSize or GroupSize must be divisible by the other");
  // Implies that GroupSize % 16 == 0
  static_assert(
      (cmp_blk_size % VecQ == 0), "cmp_blk_size must be divisible by VecQ");
  static_assert(
      (TileQ % cmp_blk_size == 0),
      "TileQ must be divisible by cmp_blk_size"); // this requirement is
                                                  // unnecessary. however, i
                                                  // enforce it to simplify the
                                                  // code
  static_assert(
      (Bits <= 4 && Bits >= 2),
      "2 <= Bits <= 4 is required"); // Bits == 1 also works. Just need to
                                     // multiply lb by 2
...
#pragma unroll
    ...
    // prefetch size = s_l2fetch_p / Bits * sizeof(XType) * TileQ / q_group_size
    // * (1 + ZeroPoint) e.g., 2KB for the same parameters above
    constexpr int32_t s_l2fetch_p = VecP * Bits;
    constexpr int32_t s_l2fetch_size =
        (s_l2fetch_p / Bits) * (TileQ / q_group_size) * (1 + ZeroPoint);
    constexpr int32_t s_l2fetch_one = (s_l2fetch_p / Bits) * (1 + ZeroPoint);

    const uint8_t* w_tile_base = w + tile_q * TileP * g / 8;
    const XType* s_tile_base =
        s + (tile_q / q_group_size) * (TileP / Bits) * (1 + ZeroPoint);

    if (!WeightsInVTCM) {
      l2fetch(
          s_tile_base + s_l2fetch_one,
          VLEN,
          VLEN,
          (s_l2fetch_size - s_l2fetch_one) * sizeof(XType) / VLEN,
          0);
    }

    if (tile_q + TileQ < VecQ) {
      constexpr int32_t l1cache_line = 64;
      for (int i = (tile_q + TileQ) / q_act_group_size;
           i < (tile_q + TileQ * 2) / q_act_group_size;
           i += l1cache_line / sizeof(float)) {
        Q6_dcfetch_A((void*)(ls + i));
      }
      for (int i = (tile_q + TileQ) / q_group_size;
           i < (tile_q + TileQ * 2) / q_group_size;
           i += l1cache_line / sizeof(float)) {
...
#pragma unroll(Bits)
    for (int32_t vec_p = 0; vec_p < TileP; vec_p += VecP) {
      // qf32
      // we should guarantee all these belong to the same bits during
      // preprocessing i.e., VecBits = VecP = VecC * 4
      ...
      const XType* s_base = s_tile_base +
          vec_p / (VecP * Bits) * (TileQ / q_group_size) * VecP *
              (1 + ZeroPoint);
      if (!WeightsInVTCM) {
        if (vec_p + VecP < TileP) {
          ...
          if (vec_p % s_l2fetch_p == 0) {
            l2fetch(
                s_base + s_l2fetch_size,
                VLEN,
                VLEN,
                s_l2fetch_size * sizeof(XType) / VLEN,
                0);
          }
        }
      }
...
#pragma unroll
      for (int32_t vec_q = 0; vec_q < TileQ; vec_q += VecQ) {
        ...
        // int16_t
        // c_vec_lo_bo_lo: even bytes of w_vec_lo_bo, c_vec_lo_bo_hi: odd bytes
        // of w_vec_lo_bo
        HVX_VectorPair c_vec_lo_bo = Q6_Wh_vlut16_VbVhR_nomatch(
            w_vec_lo_bo, lvec_arr[vec_q / VecQ], 0); // Q = 0, even lo
        HVX_VectorPair c_vec_hi_bo = Q6_Wh_vlut16_VbVhR_nomatch(
            w_vec_hi_bo, lvec_arr[vec_q / VecQ], 1); // Q = 2, even hi
        HVX_VectorPair c_vec_lo_to = Q6_Wh_vlut16_VbVhR_nomatch(
            w_vec_lo_to, lvec_arr[vec_q / VecQ], 2); // Q = 1, odd lo
        HVX_VectorPair c_vec_hi_to = Q6_Wh_vlut16_VbVhR_nomatch(
            w_vec_hi_to, lvec_arr[vec_q / VecQ], 3); // Q = 3, odd hi

        // After unroll, the boolean variables should be broadcasted to
        // constexpr and the branches will be expanded
        const bool cmp_blk_head = (vec_q % cmp_blk_size == 0);
        const bool cmp_blk_tail =
            (vec_q % cmp_blk_size == (cmp_blk_size - VecQ));
        const bool q_group_tail =
            (vec_q % q_group_size == (q_group_size - VecQ));
        ...
        if (cmp_blk_head) {
          // reset int32_t sum
          c_vec_lo =
              Q6_Ww_vadd_VhVh(Q6_V_lo_W(c_vec_lo_bo), Q6_V_lo_W(c_vec_hi_bo));
          c_vec_hi =
              Q6_Ww_vadd_VhVh(Q6_V_hi_W(c_vec_lo_bo), Q6_V_hi_W(c_vec_hi_bo));
        } else {
          c_vec_lo = Q6_Ww_vaddacc_WwVhVh(
              c_vec_lo, Q6_V_lo_W(c_vec_lo_bo), Q6_V_lo_W(c_vec_hi_bo));
          c_vec_hi = Q6_Ww_vaddacc_WwVhVh(
              c_vec_hi, Q6_V_hi_W(c_vec_lo_bo), Q6_V_hi_W(c_vec_hi_bo));
        }
        c_vec_lo = Q6_Ww_vaddacc_WwVhVh(
            c_vec_lo, Q6_V_lo_W(c_vec_lo_to), Q6_V_lo_W(c_vec_hi_to));
        c_vec_hi = Q6_Ww_vaddacc_WwVhVh(
            c_vec_hi, Q6_V_hi_W(c_vec_lo_to), Q6_V_hi_W(c_vec_hi_to));

        // qf32
        if (cmp_blk_tail) {
          const XType* s_ptr =
              s_base + (vec_q / q_group_size) * VecP * (1 + ZeroPoint);
          ...
          HVX_VectorPair s_vec_lo =
              Q6_Wqf32_vmpy_VhfVhf(s_vec_lo_fp16, ones_vec);
          HVX_VectorPair s_vec_hi =
              Q6_Wqf32_vmpy_VhfVhf(s_vec_hi_fp16, ones_vec);

          HVX_Vector ls_vec = Q6_V_vsplat_R(
              _fp32_to_bits(ls[(tile_q + vec_q) / q_act_group_size]));
          HVX_Vector lb_vec;
          if (ZeroPoint) {
            lb_vec = Q6_Vh_vsplat_R(_fp16_to_bits(
                reinterpret_cast<const __fp16*>(lb) +
                (tile_q + vec_q) / q_group_size));
          } else {
            lb_vec = Q6_V_vsplat_R(
                _fp32_to_bits(lb[(tile_q + vec_q) / q_group_size]));
          }

          // int32_t -> fp32
          // TODO: consider reordering for understanding: 0, 1, 2, 3 ->
          // lo_W(lo), hi_W(lo), lo_W(hi), hi_W(hi)
          ...
          if (q_group_tail && (vec_p % (VecP * Bits) == VecP)) {
            ...
            if (ZeroPoint) {
              ...
              HVX_Vector z_vec_hi_fp16 =
                  vmem(s_ptr + VecP + VLEN / sizeof(XType));

              HVX_VectorPair zlb_vec_lo =
                  Q6_Wqf32_vmpy_VhfVhf(z_vec_lo_fp16, lb_vec);
              HVX_VectorPair zlb_vec_hi =
                  Q6_Wqf32_vmpy_VhfVhf(z_vec_hi_fp16, lb_vec);

              c_vec_0_qf32 =
                  Q6_Vqf32_vadd_Vqf32Vqf32(c_vec_0_qf32, Q6_V_lo_W(zlb_vec_lo));
              c_vec_1_qf32 =
                  Q6_Vqf32_vadd_Vqf32Vqf32(c_vec_1_qf32, Q6_V_lo_W(zlb_vec_hi));
              c_vec_2_qf32 =
                  Q6_Vqf32_vadd_Vqf32Vqf32(c_vec_2_qf32, Q6_V_hi_W(zlb_vec_lo));
              c_vec_3_qf32 =
                  Q6_Vqf32_vadd_Vqf32Vqf32(c_vec_3_qf32, Q6_V_hi_W(zlb_vec_hi));
            } else {
          ...
          // * s
          c_vec_0_qf32 =
              Q6_Vqf32_vmpy_Vqf32Vqf32(c_vec_0_qf32, Q6_V_lo_W(s_vec_lo));
          c_vec_1_qf32 =
              Q6_Vqf32_vmpy_Vqf32Vqf32(c_vec_1_qf32, Q6_V_lo_W(s_vec_hi));
          c_vec_2_qf32 =
              Q6_Vqf32_vmpy_Vqf32Vqf32(c_vec_2_qf32, Q6_V_hi_W(s_vec_lo));
          c_vec_3_qf32 =
              Q6_Vqf32_vmpy_Vqf32Vqf32(c_vec_3_qf32, Q6_V_hi_W(s_vec_hi));
...
hvx_tbl(
  ...
  static_assert(
      (ActGroupSize == -1),
      "For BitNet model, only per-tensor quantization is supported");
  static_assert(
      !ZeroPoint, "For BitNet model, the quantization should be symmetric");
  // Implies that GroupSize % 16 == 0
  static_assert(
      (Bits <= 4 && Bits >= 2),
      "2 <= Bits <= 4 is required"); // Bits == 1 also works. Just need to
...
#pragma unroll
  ...
  HVX_Vector s_vec = Q6_V_lo_W(
      Q6_Wqf32_vmpy_VhfVhf(Q6_Vh_vsplat_R(_fp16_to_bits(s)), ones_vec));
#pragma unroll(Bits)
  for (int32_t vec_p = 0; vec_p < TileP; vec_p += VecP) {
#pragma unroll
    for (int32_t vec_c = vec_p; vec_c < vec_p + VecP;
         vec_c += VecP / sizeof(CType)) {
...
template <
    ...
    int Bits = 2>
inline typename std::enable_if_t<
    std::is_same<XType, __fp16>::value && std::is_same<CType, float>::value,
    int>
...
#pragma unroll
    ...
    HVX_Vector c_bitsum_lo =
        Q6_Vhf_equals_Wqf32(Q6_W_vcombine_VV(c_bits[2], c_bits[0]));
    HVX_Vector c_bitsum_hi =
        Q6_Vhf_equals_Wqf32(Q6_W_vcombine_VV(c_bits[3], c_bits[1]));
