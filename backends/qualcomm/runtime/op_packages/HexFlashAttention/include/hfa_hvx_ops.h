...
static inline void hvx_sub_multiple(HVX_Vector* vec_ptr, HVX_Vector opr) {
#pragma unroll
...
static inline void
hvx_mpy_multiple(HVX_Vector* vec_ptr, HVX_Vector opr, uint16_t num_vecs) {
  for (uint16_t i = 0; i < num_vecs; ++i) {
...
template <uint16_t num_vecs>
static inline HVX_Vector hvx_reduce_sum(
    HVX_Vector curr,
...
template <uint16_t num_vecs>
static inline HVX_Vector hvx_reduce_max(
    HVX_Vector curr,
