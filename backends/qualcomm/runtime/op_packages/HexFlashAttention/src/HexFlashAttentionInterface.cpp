...
static std::array<const char*, 1> sg_opNames{{"FlashAttention"}};
 ...
 * can be set by Qnn_ErrorHandle_t HexFlashAttentionLogSetLevel(QnnLog_Level_t
 * maxLogLevel)
 */
/*
...
Qnn_ErrorHandle_t HexFlashAttentionInit(
...
Qnn_ErrorHandle_t HexFlashAttentionGetInfo(const QnnOpPackage_Info_t** info) {
  if (!sg_packageInitialized)
...
Qnn_ErrorHandle_t HexFlashAttentionLogInitialize(
...
Qnn_ErrorHandle_t HexFlashAttentionLogSetLevel(QnnLog_Level_t maxLogLevel) {
  if (maxLogLevel < QNN_LOG_LEVEL_ERROR)
    return QNN_LOG_ERROR_INVALID_ARGUMENT;
  sg_maxLogLevel = maxLogLevel;
  return QNN_SUCCESS;
...
Qnn_ErrorHandle_t HexFlashAttentionLogTerminate() {
  if (!sg_logInitialized)
    return QNN_OP_PACKAGE_ERROR_LIBRARY_NOT_INITIALIZED;
...
Qnn_ErrorHandle_t HexFlashAttentionValidateOpConfig(Qnn_OpConfig_t opConfig) {
   ...
   */
  if (std::string(opConfig.v1.typeName) == "FlashAttention") {
 ...
 * HexFlashAttentionCreateKernels (QnnOpPackage_GraphInfrastructure_t
 ...
 * HexFlashAttentionFreeKernels (QnnOpPackage_Kernel_t* kernels)
 *
 * (version 1.4.0) Qnn_ErrorHandle_t HexFlashAttentionCreateOpImpl
 ...
 * HexFlashAttentionFreeOpImpl (QnnOpPackage_OpImpl_t opImpl)
 */

Qnn_ErrorHandle_t HexFlashAttentionTerminate() {
  if (!sg_packageInitialized)
    return QNN_OP_PACKAGE_ERROR_LIBRARY_NOT_INITIALIZED;
...
/* latest version */
Qnn_ErrorHandle_t HexFlashAttentionInterfaceProvider(
    QnnOpPackage_Interface_t* interface) {
  if (!interface)
    return QNN_OP_PACKAGE_ERROR_INVALID_ARGUMENT;
  interface->interfaceVersion = {1, 4, 0};
  interface->v1_4.init = HexFlashAttentionInit;
  interface->v1_4.terminate = HexFlashAttentionTerminate;
  interface->v1_4.getInfo = HexFlashAttentionGetInfo;
  interface->v1_4.validateOpConfig = HexFlashAttentionValidateOpConfig;
  interface->v1_4.createOpImpl = nullptr;
  interface->v1_4.freeOpImpl = nullptr;
  interface->v1_4.logInitialize = HexFlashAttentionLogInitialize;
  interface->v1_4.logSetLevel = HexFlashAttentionLogSetLevel;
  interface->v1_4.logTerminate = HexFlashAttentionLogTerminate;
  return QNN_SUCCESS;
}
